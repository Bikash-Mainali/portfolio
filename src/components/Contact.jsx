        ...
        <section id="contact" ref={ref} className="relative max-w-7xl mx-auto px-6 sm:px-0 sm:pt-45 pt-35">
            ...
            <div className="grid md:grid-cols-5 gap-10">
                ...
                >
                    {infos.map(item => (
                        <div key={item.label}
                             className="card-glass rounded-xl p-4 flex items-center gap-4 hover:border-accent dark:hover:border-primary-weak transition-all duration-200">
                            <span className="text-2xl">{item.icon}</span>
                            <div>
                                <div
                                    className="font-mono text-xs text-slate">{item.label}</div>
                                {item.href ? (
                                    <a href={item.href} target={item.href.startsWith('http') ? '_blank' : undefined}
                                       rel="noopener noreferrer"
                                       className="text-sm text-dark dark:text-light hover:text-accent dark:hover:text-primary transition-colors">
                                        {item.value}
                                    </a>
                                ) : (
                                    <span className="text-sm text-dark dark:text-light">{item.value}</span>
                                )}
                            </div>
                        </div>
                    ))}
                    ...
                    <form onSubmit={handleSubmit} className="card-glass rounded-2xl p-8 space-y-5">
                        <div className="grid sm:grid-cols-2 gap-5">
                            <div>
                                ...
                                <input
                                    ...
                                    required
                                    placeholder="Name"
                                    className="w-full bg-white border placeholder-light dark:placeholder-slate border-light dark:border-dark dark:bg-navy-900 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-primary-weak transition-all"
                                />
                            </div>
                            <div>
                        ...
                        <p className="font-mono text-xs text-dark dark:text-slate text-center">
                            ...
                            <a href="mailto:bikashmainali18@gmail.com"
                               className="text-accent dark:text-primary-weak hover:underline">
