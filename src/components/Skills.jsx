...
const barColor = {
    Frontend: 'bg-gradient-to-r from-teal-600 to-primary-weak dark:from-primary-weak dark:to-teal-400',
...
export default function Skills() {
    ...
    return (
        <section id="skills" ref={ref} className="relative max-w-7xl mx-auto px-6 sm:px-0 sm:pt-45 pt-35">
            <div
                className={`transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                <h2 className="section-title">Tech Stack</h2>
                <div className="section-line"></div>
                <p className="text-dark dark:text-light text-sm mb-12">A snapshot of my technical proficiency</p>
            </div>
                    ...
                    >
                        <div className="flex items-center justify-between mb-3">
                            <div className="flex items-center gap-3">
                                <span
                                    className="font-body text-stone-900 dark:text-white text-sm">{skill.name}</span>
                                <span className={`tag text-xs border px-2 py-0.5 rounded-full ${catColor[skill.cat]}`}>
                    {skill.cat}
                  </span>
                            </div>
                            <span className="font-mono text-sm text-dark dark:text-light">{skill.pct}%</span>
                        </div>
                        <div className="h-1.5 bg-light dark:bg-dark rounded-full overflow-hidden">
                            <div
