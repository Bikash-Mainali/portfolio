...
export default function BlogCard({post, onClick}) {
    ...
    const lightColor = CATEGORY_COLORS_LIGHT[categoryName];
    const darkColor = CATEGORY_COLORS_DARK[categoryName];
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col flex-1 min-w-0">
                {/* Title */}
                <h2
                    className="font-bold text-base sm:text-lg leading-snug mb-1.5 transition-colors line-clamp-2 text-amber-700 group-hover:text-accent dark:text-amber-400 dark:group-hover:text-amber-300"
                ...
                {/* Date */}
                <div className="flex items-center gap-1.5 text-xs mb-2 text-slate2">
                ...
                {/* Excerpt */}
                <p className="text-sm leading-relaxed line-clamp-2 mb-3 flex-1 dark:text-light font-light">
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                    ...
                    {tags.slice(0, 3).map((tag) => (
                        <span key={tag.id}
                              className="text-xs px-2 py-0.5 rounded-full text-slate bg-light dark:bg-dark">
                            #{tag.name}
                        </span>
                    ))}
                    {tags.length > 5 && (
                        <span className="text-xs text-slate ">+{tags.length - 3} more</span>
                    )}
                </div>
            </div>
        </div>
